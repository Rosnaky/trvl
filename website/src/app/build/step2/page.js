        ...
        <div className="mt-20">
            ...
            <div className="flex gap-4 w-1/2 mx-auto mt-10 justify-around">
            {
                options.map(([option, numPeople]) => {
                    return <div 
                        key={option}
                        className={"w-36 py-6 my-auto border border-white flex justify-center items-center text-white rounded-lg cursor-pointer hover:bg-white/10" + (selected === option ? " bg-white/20" : " bg-transparent")}
                        onClick={() => {
            ...
            <div className={"w-1/1 items-center justify-center flex flex-row"}>
                <Link href={"/build/step3"} className="flex opacity-60 hover:opacity-100 transition duration-200 text-center text-white cursor-pointer mt-10"
                    onClick={() => {
